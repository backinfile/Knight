...
public class CardGroup {
	...
	public void forEach(Consumer<Card> action) {
		for (int i = 0; i < cards.size(); i++) {
			action.accept(cards.get(i));
		}
	}

	public boolean any(Function<Card, Boolean> function) {
		for (int i = 0; i < cards.size(); i++) {
			if (function.apply(cards.get(i))) {
				return true;
			}
		}
		return false;
	...
	public boolean all(Function<Card, Boolean> function) {
		for (int i = 0; i < cards.size(); i++) {
			if (!function.apply(cards.get(i))) {
				return false;
			}
